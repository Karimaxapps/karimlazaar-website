/* ============================================================================
   ...
   (adapted for Next.js: ESM export + destroy() cleanup for client remounts)
   ----------------------------------------------------------------------------
   Vanilla JS, zero dependencies. Builds its own DOM and injects namespaced CSS
   into the container you give it. mountScrollWorld() returns { destroy }.
   ========================================================================== */
...
function mountScrollWorld(container, config) {
  ...
  SECTIONS.forEach((s, i) => {
    const dive = { kind: 'dive', si: i, clip: s.clip, clipM: s.clipMobile, still: s.still, stillM: s.stillMobile,
                   accent: s.accent, w: s.scroll || DIVE_W, linger: s.linger || 0, ease: s.ease };
    SEGMENTS.push(dive);
    s._seg = dive;
  ...
  // "dwell": hold at BOTH endpoints (start island / arrival island), sweep the
  // middle quickly — for legs that travel between scenes. Seams untouched.
  const dwellEase = (x, L) => { L = clamp(L); const s = x * x * x * (x * (x * 6 - 15) + 10); return (1 - L) * x + L * s; };
  ...
  function read() {
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      s.target = s.ease === 'dwell' ? dwellEase(local, s.linger != null && s.linger > 0 ? s.linger : 0.85)
        : s.linger ? lingerEase(local, s.linger) : local;
      let outside = 0;
    ...
    for (let i = 0; i < N; i++) {
      ...
      const PEAK = config.copyPeak != null ? config.copyPeak : 0.5;    // where mid-section copy peaks (0..1)
      let cop;
      if (i === 0) cop = after ? 0 : smooth(1 - pr / 0.62);            // greets on landing
      else if (i === N - 1) cop = before ? 0 : smooth(pr / 0.4);       // holds CTA at the end
      else cop = (before || after) ? 0 : (pr < PEAK ? smooth(pr / PEAK) : smooth((1 - pr) / (1 - PEAK)));
      const c = copies[i];
      c.style.opacity = cop;
  ...
  function destroy() {
    ...
    SEGMENTS.forEach(s => { if (s.video && s.video.src) { try { URL.revokeObjectURL(s.video.src); } catch (e) {} } });
